...
{p2colset 5 17 24 2}{...}
{p2col:{bf:goprobit} {hline 2}}Generalized ordered probit regression{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab :Model}
{synopt :{opth dist:ribution(goprobit##distname:distname)}}specify distribution for link function; default is {opt dist:ribution(normal)}{p_end}
...
{syntab :Maximization}
{synopt :{it:{help goprobit##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
...
{pstd}
{opt goprobit} fits ordered response regression models of ordinal variable
{depvar} on the independent variables {indepvars}. It differs from {opt oprobit} by offering a selection of alternate, flexible link functions to use in place of the conventional Normal CDF.
...
{phang}
{opth dist:ribution(goprobit##distname:distname)} specifies the distribution of 
...
{pstd}
{cmd:goprobit} stores the following in {cmd:e()}:
...
{synopt:{cmd:e(cmd)}}{cmd:goprobit}{p_end}
...
{pstd}
This vesion of {bf:goprobit} is fully functional, although not yet optimized in 
